...
import { useUpdateBrand } from "@/hooks/querys/brand";
...
const EditBrandModal: React.FC<EditBrandModalProps> = ({ isOpen, onClose, brand }) => {
    ...
    const onSubmit = (data: CategoryRequestType) => {
        ...
        if (brand?.brandId) {
            updateBrandMutation.mutate({ id: brand.brandId, data: formData }, {
                onSuccess: () => {
                    reset(); // Reset form
                    onClose(); // Đóng modal
                },
        ...
        <BaseModal isOpen={isOpen} onClose={onClose}>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="p-4">
                    ...
                    {/* Tên Category */}
                    <InputField {...register("name", { required: true })} label="Tên thương hiệu" id="name" name="name" type="text" />
                ...
                <div className="flex justify-end px-4 py-3 text-right border-t">
                    <SubmitButton text={updateBrandMutation.isPending ? "Đang xử lý..." : "Lưu thay đổi"} disabled={updateBrandMutation.isPending} />
                </div>
            </form>
        </BaseModal>
    );
