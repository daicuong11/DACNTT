...
import { useAddAddress } from '@/hooks/querys/address.query'
...
const AddAddressPage = () => {
  ...
  const handleSaveAddress = () => {
    ...
    const addAddressReq: AddressRequestType = {
    ...
    addAddressMutate(addAddressReq, {
      onSuccess: () => {
  ...
  return (
    <div className='flex flex-col min-h-screen p-5 pb-20 mt-1 bg-white rounded-md'>
      <div className='flex items-center'>
        ...
        <div className='text-2xl font-semibold text-black'>Tạo địa chỉ mới</div>
      </div>
      ...
      <div className='flex flex-col p-5 mt-8 gap-y-4'>
        ...
        <button disabled={isPending} onClick={handleSaveAddress} className='w-full py-2 mt-5 btn btn-danger'>
          {isPending ? <LoadingItem className='border-white' /> : 'Lưu địa chỉ'}
        </button>
      </div>
    </div>
  )
